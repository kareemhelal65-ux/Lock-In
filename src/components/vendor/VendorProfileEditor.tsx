...
export default function VendorProfileEditor({ vendorId, onClose, onProfileUpdated }: VendorProfileEditorProps) {
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 pb-safe">
            ...
            >
                <div className="p-6 border-b-2 border-cool-gray/20 flex items-center justify-between sticky top-0 bg-deep-charcoal z-10">
                    <h2 className="font-display font-black text-2xl uppercase tracking-widest text-white">Merchant Profile</h2>
                ...
                <div className="p-6 overflow-y-auto flex-1 space-y-8">
                    ...
                    <div>
                        <h3 className="font-display font-bold uppercase text-cool-gray mb-4">Visual Identity</h3>
                                ...
                                {bannerPreview ? (
                                    <div className="absolute inset-0 w-full h-full">
                                        ...
                                        <div className="absolute inset-0 flex items-center justify-center bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity">
                                            <span className="font-bold text-white uppercase text-sm tracking-wider">Change Banner</span>
                                        </div>
                                    </div>
                                ) : (
                                    <>
                                        <Upload className="w-8 h-8 text-cool-gray mb-2" />
                                        <span className="font-bold text-cool-gray uppercase text-sm tracking-wider">Upload Banner Image</span>
                                    </>
                                )}
                                <input type="file" ref={bannerInputRef} className="hidden" accept="image/*" onChange={(e) => handleFileChange(e, 'banner')} />
                            </div>

                            <div className="flex flex-col sm:flex-row gap-6 items-start sm:items-end -mt-12 sm:-mt-16 ml-4 sm:ml-6 relative z-10">
                                {/* Logo Upload */}
                                <div
                                    ...
                                    {logoPreview ? (
                                        <>
                                            ...
                                            <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity">
                                                ...
                                                <span className="font-bold text-white uppercase text-[10px] tracking-wider">Change</span>
                                            </div>
                                        </>
                                    ) : (
                                        <>
                                            <Upload className="w-6 h-6 text-cool-gray mb-1" />
                                            <span className="font-bold text-cool-gray uppercase text-[10px] text-center px-2 tracking-wider">Upload Logo</span>
                                        </>
                                    )}
                                ...
                                <div className="flex-1 w-full pb-2">
                                    <label className="block text-xs font-bold uppercase tracking-wider text-cool-gray mb-2">Display Name</label>
                                    <input
                                        type="text"
                                        value={name}
                                        ...
                                        placeholder="Enter Store Name (Optional update)"
                                    />
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="pt-6 border-t border-cool-gray/20">
                        <h3 className="font-display font-bold uppercase text-cool-gray mb-4">Financial Details</h3>
                        <div className="space-y-4">
                            <div>
                                <label className="block text-xs font-bold uppercase tracking-wider text-cool-gray mb-2">InstaPay Address (GPA)</label>
                                <input
                                    type="text"
                            ...
                            <div>
                                <label className="block text-xs font-bold uppercase tracking-wider text-cool-gray mb-2">Verified Full Name</label>
                                <input
                                    type="text"
                                    value={instapayName}
                                    onChange={(e) => setInstapayName(e.target.value)}
                                    className="w-full bg-zinc-900 border-2 border-cool-gray/30 rounded-xl p-3 sm:p-4 font-display font-bold text-lg focus:border-volt-green focus:outline-none transition-colors"
                                    placeholder="Optional updated name"
                                />
                            </div>
                        </div>
                    </div>
                ...
                <div className="p-6 border-t-2 border-cool-gray/20 bg-zinc-900/50 sticky bottom-0 z-10">
                    <AnimatePresence mode="wait">
                        {status === 'error' && (
                            ...
                            >
                                <AlertCircle className="w-5 h-5 flex-shrink-0" /> Failed to update profile. Please try again.
                            </motion.div>
                        )}
                        {status === 'success' && (
                            <motion.div
                    ...
                    >
                        {isLoading ? (
                            ...
                            <span className="flex items-center gap-2">Saved <CheckCircle2 className="w-5 h-5" /></span>
                        ) : 'Save Profile'}
                    </button>
                    <p className="text-center text-xs text-cool-gray mt-4 font-medium uppercase tracking-wider">
                        Changes requiring verification may take up to 24 hours to reflect on Consumer devices.
                    </p>
                </div>
            </motion.div>
        </div>
